...
img_ori = imutils.resize(img_ori, height=IMG_RESIZE)
...
def on_mouse(event, x, y, flag, param):
    if event == cv2.EVENT_LBUTTONDOWN :
        rectpos.append([[x,y]])
        cv2.circle(img_ori, (x,y), 3, (0,255,0), -1)
        cv2.imshow("image", img_ori)
        if len(rectpos) == 4:
            cv2.destroyAllWindows()

cv2.namedWindow('image')
cv2.setMouseCallback('image', on_mouse, img_ori)


cv2.imshow("image", img_ori)
cv2.waitKey(0)

img_ori = four_point_transform(img_ori, np.array(rectpos).reshape(4,2))
...
img_blurred = cv2.GaussianBlur(gray, ksize=(5, 5), sigmaX=BLUR_BIAS)

if THERSH_HOLD_GLOBAL == False:
...
for c in digitCnts:
    ...
    print(x,y,w,h)
    for seg in segments:
        cv2.rectangle(img_ori, (seg[0][0]+x,seg[0][1]+y), (seg[1][0]+x,seg[1][1]+y), (0, 255, 0), 1)
    cv2.rectangle(img_ori, (x,y),(x+w,y+h), (0, 255, 0), 1)
    cv2.putText(img_ori, str(digit), (x - 10, y - 10),
